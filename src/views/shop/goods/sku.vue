		...
		<template v-if="skus_type === 0">
			<el-form ref="form" label-width="80px">
				<el-form-item label="市场价格">
					<el-input type="number" v-model="sku_value.oprice" style="width: 165px;">
						<template slot="append">元</template>
					</el-input>
				</el-form-item>
				<el-form-item label="销售价格">
					<el-input type="number" v-model="sku_value.pprice" style="width: 165px;">
						<template slot="append">元</template>
					</el-input>
				</el-form-item>
				<el-form-item label="成本价格">
					<el-input type="number" v-model="sku_value.cprice" style="width: 165px;">
						<template slot="append">元</template>
					</el-input>
				</el-form-item>
				<el-form-item label="商品重量">
					<el-input type="number" v-model="sku_value.weight" style="width: 165px;">
						<template slot="append">公斤</template>
					</el-input>
				</el-form-item>
				<el-form-item label="商品体积">
					<el-input type="number" v-model="sku_value.volume" style="width: 165px;">
	...
	export default {
		...
		created() {
			...
			this.axios.get('/admin/goods/read/'+this.id,{
				...
				this.vModel('sku_card',data.goodsSkuCard.map(item=>{
					item.list = item.goodsSkuCardValue
					item.list.map(v=>{
		...
		methods: {
			...
			submit(){
				let list = []
				if(this.$refs.table){
					list = this.$refs.table.list.map(v=>{
						v.goods_id = this.id
						return v
					}) 
				}
